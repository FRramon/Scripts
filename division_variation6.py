...
def division_ICA(pinfo, case, step):
    ...
    # Division on the left side : Automatic with the minimization of the
    # distance between ICA_MCA and ACA
    ...
    # On the right side : Manual division because there is no right ACA A1

    fig = plt.figure(figsize=(7, 7))
    ax = fig.add_subplot(111, projection='3d')
    ax.grid()
    ax.scatter(points_LICAMCA[:, 0], points_LICAMCA[:, 1],
               points_LICAMCA[:, 2], label='LEFT ICA')
    ax.scatter(points_RICAMCA[:, 0], points_RICAMCA[:, 1],
               points_RICAMCA[:, 2], label='RIGHT ICA ')
    ...
    X, Y, Z = points_RICAMCA[:, 0], points_RICAMCA[:, 1], points_RICAMCA[:, 2]
    L = np.linspace(0, len(points_RICAMCA) - 1, 20)
    ...
    for i in range(len(points_RICAMCA)):
        print('   ', i, ' : point ', i)

    target = int(input('-->  '))

    points_RICA = points_RICAMCA[:target]
    points_RMCA = points_RICAMCA[target:]
...
# FINAL VUSUALIZATION
    ...
    ax.scatter(points_LICA[:, 0], points_LICA[:, 1],
               points_LICA[:, 2], c='r', label='LEFT ICA')
...
# Creation of the dictionary of the new arteries
...
def division_ACAs(pinfo, case, step):
    ...
    ax.grid(False)
    ax.scatter(points_laca[:, 0], points_laca[:, 1],
               points_laca[:, 2], label='L_ACA')
    ax.scatter(points_raca[:, 0], points_raca[:, 1],
               points_raca[:, 2], label='R_ACA')
    ax.legend()
    # ax.view_init(30,30)

    # DIVISION L_ACA INTO LACA A1 & LACA A2

    points_RA2 = points_raca
    ...
    for i in range(points_laca.shape[0]):
        # Norm between first/last  LACA points and LICAMCA points
        norm_end = np.linalg.norm(target[1] - points_laca[i])
        norm_start = np.linalg.norm(target[0] - points_laca[i])
        lnorms_end.append(norm_end)
        lnorms_start.append(norm_start)
    # Min of the two lists
    Ltot_norms = lnorms_end + lnorms_start
    ...
    ax.scatter(points_acom[:, 0], points_acom[:, 1],
               points_acom[:, 2], c='g', label='Acom')
    ax.scatter(points_LA2[:, 0], points_LA2[:, 1],
               points_LA2[:, 2], c='b', label='LACA A2')
    ax.scatter(points_LA1[:, 0], points_LA1[:, 1],
               points_LA1[:, 2], c='k', label='LACA A1')
    ax.scatter(points_RA2[:, 0], points_RA2[:, 1],
               points_RA2[:, 2], c='r', label='RACA A2')
